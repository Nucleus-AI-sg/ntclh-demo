...
interface EnrolmentTabProps {
  assessment: Assessment | undefined
  trainee: Trainee
  programmes: Programme[]
}

export function EnrolmentTab({ assessment, trainee, programmes }: EnrolmentTabProps) {
  if (!assessment) {
    return <p className="text-sm text-slate-400 py-8 text-center">No assessment data available</p>
  }

  const recProg = programmes.find((p) => p.id === assessment.recommendedProgrammeId)
  const reqProg = programmes.find((p) => p.id === assessment.requestedProgrammeId)

  return (
    <div className="space-y-4">
      {/* Application Summary */}
      <div className="bg-white border border-slate-100 rounded-xl p-4 shadow-sm">
        <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-4">Application Summary</h3>
        <dl className="grid grid-cols-2 lg:grid-cols-4 gap-4 text-sm">
          <div>
            <dt className="text-[10px] font-bold text-slate-400 uppercase">Submitted</dt>
            <dd className="font-semibold text-slate-800">{trainee.applicationDate}</dd>
          </div>
          <div>
            <dt className="text-[10px] font-bold text-slate-400 uppercase">Channel</dt>
            <dd className="font-semibold text-slate-800">FormSG {trainee.singpassVerified ? '(SingPass)' : ''}</dd>
          </div>
          <div>
            <dt className="text-[10px] font-bold text-slate-400 uppercase">Requested Course</dt>
            <dd className="font-semibold text-slate-800">{reqProg?.shortName ?? assessment.requestedProgrammeId}</dd>
          </div>
          <div>
            <dt className="text-[10px] font-bold text-slate-400 uppercase">Status</dt>
            <dd className="font-semibold text-slate-800 capitalize">{assessment.status.replace(/_/g, ' ')}</dd>
          </div>
        </dl>
      </div>
        ...
        <div className="flex items-start justify-between relative z-10">
          <div>
            <h3 className="text-xs font-black text-blue-700 uppercase tracking-widest mb-4 flex items-center">
              <Briefcase className="h-4 w-4 mr-2" /> AI Suitability Assessment
            </h3>
          ...
          <div className="flex flex-col items-center">
            ...
            <span className="mt-2 text-[10px] font-bold text-blue-600 bg-white px-2 py-0.5 rounded shadow-sm uppercase">
            ...
            <ul className="space-y-2">
              {assessment.reasoning.map((r, i) => (
                <li key={i} className="flex items-start text-xs text-slate-700">
                  <CheckCircle className="h-3.5 w-3.5 text-blue-500 mt-0.5 mr-2 flex-shrink-0" /> {r}
                </li>
              ))}
            </ul>
          </div>
